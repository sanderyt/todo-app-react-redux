    ...
    <div>
      <div className="login">
        <div className="login__box">
          ...
          <br />
          <button
            className={isLoading ? "btn btn--green" : "btn"}
            onClick={loginHandler}
          >
            Inloggen {isLoading && <LoadingSpinner />}
          </button>
          <br />
          {errorLogin && <p className="error">{errorLogin}</p>}
          <p>
            <h3 onClick={clickRegister}>Registeren</h3>| Wachtwoord vergeten
          </p>
        </div>
      </div>
      {registerClick && (
        <Modal title="Maak een account aan" close={clickRegister}>
          ...
          >
            Registeren {isLoading && <LoadingSpinner />}
          </button>
        </Modal>
      )}
    </div>
